...
func setArchived(s Session, archived bool, config, data string) (Session, error) {
	...
	for i, n := range names {
		if err := move(filepath.Join(from, n), filepath.Join(to, n)); err != nil {
			for _, done := range names[:i] {
				if uerr := move(filepath.Join(to, done), filepath.Join(from, done)); uerr != nil {
...
func isLive(config, uuid string) (bool, error) {
	...
	for _, f := range files {
		...
		// An unreadable record might be this session's, so liveness is unknown.
		if err := json.Unmarshal(b, &rec); err != nil {
			return false, fmt.Errorf("%s: %w", f, err)
		}
		if rec.SessionID == uuid && pidRunning(rec.PID) {
			return true, nil
		}
	}
	return false, nil
