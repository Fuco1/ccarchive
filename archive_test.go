...
func failRename(t *testing.T, fails func(src string) bool, err error) {
	t.Helper()
	t.Cleanup(func() { rename = renameNoReplace })
...
func TestArchiveRefusesWhenASessionRecordIsUnreadable(t *testing.T) {
	st, m := newStore(t)
	write(t, filepath.Join(st.cfg, "sessions", "1.json"), `{"pid":1,"sessionId":"`, time.Now())
	m, _ = press(t, m, "a")
	if m.err == nil || !strings.Contains(m.View(), "1.json") {
		t.Fatalf("unreadable record not refused:\n%s", m.View())
	}
	mustExist(t, st.active(uuidA+".jsonl"))
	mustNotExist(t, st.archived(uuidA+".jsonl"))
}
